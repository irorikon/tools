/*
 * @Author: iRorikon
 * @Date: 2023-04-26 11:31:28
 * @FilePath: \api-service\service\download\download.go
 */
package download
...
func Download(ctx context.Context, c *DownloadConfig, opts ...DownloadOption) error {
	...
	if err := os.MkdirAll(partialDir, 0755); err != nil {
		return errors.Wrap(err, "failed to create download location")
	}
...
func parallelDownload(ctx context.Context, c *parallelDownloadConfig) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to check progress")
	}
...
func (t *task) download(req *http.Request, bar *pb.ProgressBar) error {
	...
	output, err := os.OpenFile(t.destPath(), os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644)
	if err != nil {
		return errors.Wrapf(err, "failed to open file: %q", t.String())
	}
	defer output.Close()
...
func bindFiles(c *DownloadConfig, partialDir string) error {
	fmt.Fprintln(stdout, "\nbinding with files...")
	destPath := filepath.Join(partialDir, c.Filename)
